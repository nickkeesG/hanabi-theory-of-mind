...
class OutOfGraphReplayMemory(object):
  ...
  def __init__(self, num_actions, observation_size, stack_size, replay_capacity,
    ...
    self.pactions = np.empty((replay_capacity), dtype=np.int32)
  ...
  def add(self, observation, action, paction, reward, terminal, legal_actions):
    ...
    if self.is_empty() or self.terminals[self.cursor() - 1] == 1:
      ...
      for _ in range(self._stack_size - 1):
        self._add(dummy_observation, 0, 0, 0, 0, dummy_legal_actions)
    self._add(observation, action, paction, reward, terminal, legal_actions)

  def _add(self, observation, action, paction, reward, terminal, legal_actions):
    cursor = self.cursor()
    self.observations[cursor] = observation
    self.actions[cursor] = action
    self.pactions[cursor] = paction
  ...
  def sample_transition_batch(self, batch_size=None, indices=None):
    ...
    paction_batch = self.pactions[indices]
    ...
    return (self._state_batch, action_batch, paction_batch, reward_batch,
  ...
  def __init__(self,
    ...
    with tf.name_scope('replay'):
      with tf.name_scope('add_placeholders'):
        ...
        self.add_paction_ph = tf.placeholder(tf.int32, [], name='add_paction_ph')
      ...
      add_transition_ph = [
          self.add_obs_ph, self.add_action_ph, self.add_paction_ph, self.add_reward_ph,
      ...
      with tf.device('/cpu:*'):
        ...
        self.transition = tf.py_func(
            ...
            [tf.uint8, tf.int32, tf.int32, tf.float32, tf.uint8, tf.uint8, tf.int32,
             tf.float32],
        ...
        if use_staging:
          ...
          (states, actions, pactions, rewards, next_states,
          ...
          self.prefetch_batch = prefetch_area.put(
              (states, actions, pactions, rewards, next_states, terminals, indices,
               next_legal_actions))
        else:
      ...
      (self.states, self.actions, self.pactions, self.rewards, self.next_states,
