...
class ObservationStacker(object):
  ...
  def __init__(self, history_size, observation_size, belief_sizes, num_players):
    ...
    self._b0_size = belief_sizes[0]
    self._bc_size = belief_sizes[1]
    self._bm_size = belief_sizes[2]
    self._num_players = num_players
    self._obs_stacks = list()
    for _ in range(0, self._num_players):
      self._obs_stacks.append(np.zeros(self._observation_size *
                                       self._history_size))
    self._b0_stacks = list()
    for _ in range(0, self._num_players):
      self._b0_stacks.append(np.zeros(self._b0_size *
                                       self._history_size))
    self._bc_stacks = list()
    for _ in range(0, self._num_players):
      self._bc_stacks.append(np.zeros(self._bc_size *
                                       self._history_size))
    self._bm_stacks = list()
    for _ in range(0, self._num_players):
      self._bm_stacks.append(np.zeros(self._bm_size *
                                       self._history_size))
    
  def add_observation(self, observation, belief_zero, belief_char, belief_ment, current_player):
    ...
    self._b0_stacks[current_player] = np.roll(self._b0_stacks[current_player],
                                               -self._b0_size)
    self._b0_stacks[current_player][(self._history_size - 1) *
                                     self._b0_size:] = belief_zero

    self._bc_stacks[current_player] = np.roll(self._bc_stacks[current_player],
                                               -self._bc_size)
    self._bc_stacks[current_player][(self._history_size - 1) *
                                     self._bc_size:] = belief_char

    self._bm_stacks[current_player] = np.roll(self._bm_stacks[current_player],
                                               -self._bm_size)
    self._bm_stacks[current_player][(self._history_size - 1) *
                                     self._bm_size:] = belief_ment
  ...
  def get_observation_stack(self, current_player):
    ...
    Args:
      current_player: int, current player id.
    """
    observation_vector = np.concatenate((self._obs_stacks[current_player], self._b0_stacks[current_player],
      self._bc_stacks[current_player], self._bm_stacks[current_player]))
  ...
  def reset_stack(self):
    ...
    for i in range(0, self._num_players):
      ...
      self._b0_stacks[i].fill(0.0)
      self._bc_stacks[i].fill(0.0)
      self._bm_stacks[i].fill(0.0)
...
def parse_observations(observations, belief_0s, belief_chars, belief_ments, num_actions, obs_stacker):
  ...
  b0_vec = belief_0s[current_player]
  bc_vec = belief_chars[current_player]
  bm_vec = belief_ments[current_player]

  obs_stacker.add_observation(observation_vector, b0_vec, bc_vec, bm_vec, current_player)
...
def run_one_phase(agent, environment, obs_stacker, min_steps, statistics,
  ...
  while step_count < min_steps:
    ...
    num_episodes += 1
    sum_pred_acc += pred_acc

  return step_count, sum_returns, num_episodes, (sum_pred_acc / num_episodes)
...
def run_one_iteration(agent, environment, obs_stacker,
  ...
  number_steps, sum_returns, num_episodes, pred_acc = (
  ...
  tf.logging.info('Average prediction accuracy: %.2f', pred_acc)
  ...
  if evaluate_every_n is not None and iteration % evaluate_every_n == 0:
    ...
    eval_episode_length, eval_episode_return, eval_episode_pred_acc = map(np.mean, zip(*episode_data))
