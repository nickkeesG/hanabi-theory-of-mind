...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    self.observation_size = observation_size + 512 + 8*2
    ...
    with tf.device(tf_device):
      ...
      states_shape = (1, self.observation_size, stack_size)
      ...
      self._q, self._belief_0, self._belief_char, self._belief_ment, self._a = online_convnet(
          state=self.state_ph, num_actions=self.num_actions)
      self._replay = self._build_replay_memory(use_staging)
      self._replay_qs, _, self._replay_chars, _, self._replay_as = online_convnet(self._replay.states, self.num_actions)
      self._replay_next_qt, _, self._replay_next_chart, _, _ = target_convnet(self._replay.next_states,
                                            self.num_actions)
  ...
  def begin_episode(self, current_player, legal_actions, observation):
    ...
    self.action, self.belief_0, self.belief_char, self.belief_ment, self.pred_vec = self._select_action(
  ...
  def _select_action(self, observation, legal_actions):
    ...
    action, belief_0, belief_char, belief_ment, pred_vec = self._sess.run([self._q_argmax, 
                                self._belief_0, self._belief_char, self._belief_ment, self._a],
                                {self.state_ph: self.state,
                                  self.legal_actions_ph: legal_actions})
    assert legal_actions[action] == 0.0, 'Expected legal action.'
    
    pred_vec[0] = np.exp(pred_vec[0]) / sum(np.exp(pred_vec[0]))

    if random.random() <= epsilon:
      # Choose a random action with probability epsilon.
      legal_action_indices = np.where(legal_actions == 0.0)
      random_act = np.random.choice(legal_action_indices[0])
      return random_act, belief_0[0], belief_char[0], belief_ment[0], pred_vec[0][random_act]
    else:
      return action, belief_0[0], belief_char[0], belief_ment[0], pred_vec[0][action]
  ...
  def _store_transition(self, observation, action, paction, reward, is_terminal,
                        legal_actions):
    ...
    if not self.eval_mode:
      self._sess.run(
          self._replay.add_transition_op, {
              ...
              self._replay.add_paction_ph: paction,
